...
/// <dictionary target='member'>lbl,mouseenter,mouseleave</dictionary>
...
var Path = {
    ...
    }, root: function (a) { "use strict"; Path.routes.root = a; }, rescue: function (a) { "use strict"; Path.routes.rescue = a; }, history: { initial: {}, pushState: function (a, b, c) { "use strict"; if (Path.history.supported) { if (Path.dispatch(c)) { history.pushState(a, b, c); } } else { if (Path.history.fallback) { window.location.hash = "#" + c; } } }, popState: function (a) { "use strict"; var b = !Path.history.initial.popped && location.href === Path.history.initial.URL; Path.history.initial.popped = true; if (b) { return; } Path.dispatch(document.location.pathname); }, listen: function (a) { "use strict"; Path.history.supported = !!(window.history && window.history.pushState); Path.history.fallback = a; if (Path.history.supported) { Path.history.initial.popped = "state" in window.history, Path.history.initial.URL = location.href; window.onpopstate = Path.history.popState; } else { if (Path.history.fallback) { for (route in Path.routes.defined) { if (route.charAt(0) !== "#") { Path.routes.defined["#" + route] = Path.routes.defined[route]; Path.routes.defined["#" + route].path = "#" + route; } } Path.listen(); } } } }, match: function (a, b) {
        "use strict";
    ...
    }, dispatch: function (a) { "use strict"; var b, c; if (Path.routes.current !== a) { Path.routes.previous = Path.routes.current; Path.routes.current = a; c = Path.match(a, true); if (Path.routes.previous) { b = Path.match(Path.routes.previous); if (b !== null && b.do_exit !== null) { b.do_exit(); } } if (c !== null) { c.run(); return true; } else { if (Path.routes.rescue !== null) { Path.routes.rescue(); } } } }, listen: function () { "use strict"; var a = function () { "use strict"; Path.dispatch(location.hash); }; if (location.hash === "") { if (Path.routes.root !== null) { location.hash = Path.routes.root; } } if ("onhashchange" in window && (!document.documentMode || document.documentMode >= 8)) { window.onhashchange = a; } else { setInterval(a, 50); } if (location.hash !== "") { Path.dispatch(location.hash); } }, core: { route: function (a) { "use strict";  this.path = a; this.action = null; this.do_enter = []; this.do_exit = null; this.params = {}; Path.routes.defined[a] = this; } }, routes: { current: null, root: null, rescue: null, previous: null, defined: {} }
}; Path.core.route.prototype = { to: function (a) { "use strict"; this.action = a; return this; }, enter: function (a) { "use strict"; if (a instanceof Array) { this.do_enter = this.do_enter.concat(a); } else { this.do_enter.push(a); } return this; }, exit: function (a) { "use strict"; this.do_exit = a; return this; }, partition: function () { "use strict"; var a = [], b = [], c = /\(([^}]+?)\)/g, d, e; while (d === c.exec(this.path)) { a.push(d[1]); } b.push(this.path.split("(")[0]); for (e = 0; e < a.length; e++) { b.push(b[b.length - 1] + a[e]); } return b; }, run: function () { "use strict"; var a = false, b, c, d; if (Path.routes.defined[this.path].hasOwnProperty("do_enter")) { if (Path.routes.defined[this.path].do_enter.length > 0) { for (b = 0; b < Path.routes.defined[this.path].do_enter.length; b++) { c = Path.routes.defined[this.path].do_enter[b](); if (!c) { a = true; break; } } } } if (!a) { Path.routes.defined[this.path].action(); } } };
...
function getHash(){
	var hashes = window.location.hash.substring(2);
		var rel;
	if (deeplink){
		if (hashes === "store_sync_and_share"){
			rel = 1;
...
$("#rightarrow").animate({ "width": (handles / 3) + "%", "right": (handles / 3.5) + "%" }, pageSlideDuration, easing);
		...
		$("#main_wrapper .c-" + thisid.toString()).addClass("active");
				...
				//// set state
		window.location.hash = "/" + $(".column_content.c_" + thisid.toString()).attr("id");
...
$(document).keydown(function (event){
   ...
   event.preventDefault();
	...
	if ($.inArray(l.toString(), hidArr) < 0) {
	cid = l;
	l = 0;
	}
	}}
